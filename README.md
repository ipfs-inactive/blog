...
_**Note**: Blog posts are only displayed (at their own URLs and in the index) once the date listed in their YAML front matter has arrived. This means it's not possible to preview a post set to a future date._ 
...
- `date` - the "_published at_" date, shown on the [blog index page](https://blog.ipfs.io) please update at posting time to reflect current date (if you postdate a post in advance of publication, it will not appear when you preview the site; only posts with current and past dates are displayed) - **required**
