...
The main implementation difference between the WAN and LAN DHTs are the acceptance criteria for peers—which are eligible to be part of a routing table or query and which are not. The WAN DHT’s criteria is “do you look like a public address” and the LAN DHT’s criteria is “do you look like a non-public address.” However, while WAN DHT nodes switch from client to server mode based on whether they are publicly dialable, nodes are always LAN DHT servers (unless the “dhtclient” option has been set).
...
3. When a query to a peer completes add those results to the query queue, and pull the next peer off the queue and query them
4. The query terminates whenever the closest Beta peers to `X` have been successfully queried (i.e. no dial timeouts, errors, etc.).
...
Throughout the development process we ran many Testground tests to get an understanding of how our changes have improved the network. Below is a comparison of the performance of a 1000 peer network where all peers have around 100-120ms latencies from each other, that is running the DHT from go-ipfs v0.4.23 and the DHT from go-ipfs v0.5.0. A few notes: The v0.4.23 DHT had a few modifications to make testing easier like removing hard coded lookup timeouts, so we see just how long the queries should really be running for. Additionally, while the v0.4.23 test had 60% of the peers be undialable, the v0.5.0 test had 0 undialable nodes. This change actually benefits the v0.4.23 results, since in v0.5.0 if 60% of the nodes are undialable then, because v0.5.0 undialable nodes no longer advertise themselves as DHT servers, the network itself effectively shrinks to 40% of its size, which decreases lookup times.
